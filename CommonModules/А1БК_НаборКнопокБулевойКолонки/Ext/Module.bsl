Функция ДобавитьОписание(МассивОписаний, Знач ИмяКомпонента = Неопределено, ПутьКТаблице, ИмяКолонки, РодительЭлемента = Неопределено, ПередЭлементом = Неопределено, Параметры = Неопределено, Действия = Неопределено) Экспорт  
	...
	Если Параметры.Свойство("ОбработкаНажатия") Тогда
		ОбработкаНажатия = Параметры.ОбработкаНажатия;
	Иначе
		ОбработкаНажатия = Неопределено;
	КонецЕсли;
	...
	"ОбработкаНажатия", ОбработкаНажатия
	));
...
#Если Клиент Тогда
	...
	Функция ИзменитьЗначениеКолонки(Форма, Команда, Значение)
		...
		КонецЦикла;
		Если НЕ Настройки.ОбработкаНажатия = Неопределено Тогда
			Выполнить(Настройки.ОбработкаНажатия + "(Форма, Команда, Значение)");
		КонецЕсли;
	КонецФункции 
#КонецЕсли
