...
#Если НЕ Клиент Тогда
	...
	//		НаборЗаписей			     - Булево    - (НЕСТАБИЛЬНО!) только для независимых регистров сведений (регистры накопления всегда Истина). Определяет, будут ли записаны изменения в цикле менеджером записи или единым набором записей. По умолчанию Ложь (для обратной совместимости).
	...
	//		ОбработкаПериодичности  	 - Булево 	 - для периодических регистров. Если Истина, то выводит СрезПоследних(), иначе все данные в чистом виде. По умолчанию Истина. 
	...
	Функция ДобавитьОписание(МассивОписаний, ИмяКомпонента, Знач ИмяМетаданных, РодительЭлемента = Неопределено, ПередЭлементом = Неопределено, Параметры = Неопределено, Действия = Неопределено) Экспорт
		...
		КонецЕсли;
		ОбработкаПериодичности = А1Э_Иерархии.ЗначениеСвойства(Параметры, "ОбработкаПериодичности") <> Ложь;
		Если ТипМетаданных = "регистрсведений" Тогда
			ОбъектМетаданных = А1Э_Метаданные.ОбъектМетаданных(ИмяМетаданных);
			Если ОбъектМетаданных.ПериодичностьРегистраСведений = Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.ПозицияРегистратора
				И ОбработкаПериодичности Тогда
			...
			КонецЕсли;
			НезависимыйРегистрСведений = А1Э_РегистрыСведений.Независимый(ОбъектМетаданных);
		КонецЕсли;
		...
		"НезависимыйРегистрСведений", НезависимыйРегистрСведений, 
		"ОбработкаПериодичности", ОбработкаПериодичности,
		...
		Если ТипМетаданных = "регистрнакопления"
			Или (ТипМетаданных = "регистрсведений" И НЕ НезависимыйРегистрСведений) Тогда
			РегистраторКакКлюч = НастройкиКомпонента.Ключи.Свойство("Регистратор");
			Если НЕ РегистраторКакКлюч Тогда
				А1Э_Служебный.СлужебноеИсключение("А1БК_РедактируемаяТаблицаДанных: для регистра накопления или подчиненного регистратору регистра сведений регистратор должен быть ключом!");
			КонецЕсли;
		КонецЕсли;
		
		Колонки = Новый Массив;
		Если ТипМетаданных = "регистрнакопления" Тогда
			ДобавитьКолонку(Колонки, ИмяКомпонента, НастройкиКомпонента, А1Э_Структуры.Создать(
			"Имя", "НомерСтроки",
			"Тип", "Число:6",
			"Заголовок", "N",
			"ЭтоИзмерение", Истина,
		...
		КонецЕсли;
		Если ТипМетаданных = "регистрсведений" Тогда
			Если НастройкиКомпонента.ПериодическийРегистрСведений И НЕ НастройкиКомпонента.ОбработкаПериодичности Тогда
				ДобавитьКолонку(Колонки, ИмяКомпонента, НастройкиКомпонента, А1Э_Структуры.Создать(
				"Имя", "Период",
				"Тип", "Дата",
				"Заголовок", "Период",
				"ЭтоИзмерение", НезависимыйРегистрСведений,
				));
			КонецЕсли;
			Если НЕ НезависимыйРегистрСведений Тогда
				ДобавитьКолонку(Колонки, ИмяКомпонента, НастройкиКомпонента, А1Э_Структуры.Создать(
				"Имя", "НомерСтроки",
				"Тип", "Число:6",
				"Заголовок", "N",
		...
		Для Каждого КолонкаРегистра Из АктуальныеКолонкиРегистра(НастройкиКомпонента) Цикл
			ДобавитьКолонку(Колонки, ИмяКомпонента, НастройкиКомпонента, КолонкаРегистра);
		КонецЦикла;
		Если А1Э_Структуры.Свойство(Параметры, "ДополнительныеКолонки") Тогда
			Для Каждого ДополнительнаяКолонка Из Параметры.ДополнительныеКолонки Цикл
				ДобавитьКолонку(Колонки, ИмяКомпонента, НастройкиКомпонента, ДополнительнаяКолонка); 
			КонецЦикла;
		КонецЕсли;
		...
		"ИзменятьСоставСтрок", НастройкиКомпонента.ПериодическийРегистрСведений <> Истина Или НастройкиКомпонента.ОбработкаПериодичности <> Истина,
		);
	...
	Функция ДобавитьОписаниеСПодчинениемОбъектуИАвтонумерацией(МассивОписаний, ИмяКомпонента, Знач ИмяМетаданных, ИмяРеквизитаСсылка, ИмяРеквизитаНомерСтроки, РодительЭлемента = Неопределено, ПередЭлементом = Неопределено, Параметры = Неопределено, Действия = Неопределено) Экспорт 
		РабочиеПараметры = А1Э_Структуры.Скопировать(Параметры);
		РабочиеПараметры.Вставить("Контекст", А1Э_Структуры.Скопировать(А1Э_Структуры.ЗначениеСвойства(РабочиеПараметры, "Контекст")));
		РабочиеПараметры.Контекст.Вставить("Автонумерация__ИмяРеквизита", ИмяРеквизитаНомерСтроки);	
		
		РабочиеДействия = А1Э_Структуры.Скопировать(Действия);
		А1Э_УниверсальнаяФорма.ДобавитьОбработчикКДействиям(РабочиеДействия, "ПриНачалеРедактирования", ИмяМодуля() + ".АвтонумерацияПриНачалеРедактирования", 0);
		
		Возврат ДобавитьОписаниеСПодчинениемОбъекту(МассивОписаний, ИмяКомпонента, ИмяМетаданных, ИмяРеквизитаСсылка, РодительЭлемента, ПередЭлементом, РабочиеПараметры, РабочиеДействия);	
	КонецФункции
	...
	Функция ЗагрузитьДанные(Форма, ИмяКомпонента) Экспорт 
		...
		Если НЕ ЗначениеЗаполнено(Запрос.Текст) Тогда 
			...
			Если НастройкиКомпонента.ТипМетаданных = "регистрсведений" И НастройкиКомпонента.ПериодическийРегистрСведений И НастройкиКомпонента.ОбработкаПериодичности Тогда
		...
		Если ТаблицаЗначений.Количество() > 0 Тогда
			Для Каждого Колонка Из НастройкиКомпонента.Колонки Цикл
				...
				ИмяКолонкиИсходногоЗначения = ИмяКолонкиИсходногоЗначения(ИмяКолонки); 
				ТаблицаЗначений.Колонки.Добавить(ИмяКолонкиИсходногоЗначения, ТаблицаЗначений.Колонки[ИмяКолонки].ТипЗначения);
				ТаблицаЗначений.ЗагрузитьКолонку(ТаблицаЗначений.ВыгрузитьКолонку(ИмяКолонки), ИмяКолонкиИсходногоЗначения);	
			КонецЦикла;
			Форма[ИмяКомпонента].Загрузить(ТаблицаЗначений);
		КонецЕсли;
...
#Если НЕ Клиент Тогда
	...
	Функция ДобавитьКолонку(Колонки, ИмяКомпонента, НастройкиКомпонента, ИсточникДанных) 
		...
		ОписаниеКолонки = ОписаниеКолонки(ДанныеДляОписания, ИмяКомпонента);
		Колонки.Добавить(ОписаниеКолонки);
		Колонки.Добавить(ОписаниеКолонкиИсходныхЗначений(ДанныеДляОписания, ИмяКомпонента));
	...
	Функция ОписаниеКолонки(ИсточникДанных, ИмяКомпонента)
		Если ТипЗнч(ИсточникДанных) = Тип("ОбъектМетаданных") Тогда
			...
			"СвязиПараметровВыбора", СвязиПараметровВыбораДляТаблицы(ИсточникДанных, ИмяКомпонента));
	...
	Функция СвязиПараметровВыбораДляТаблицы(ИсточникДанных, ИмяКомпонента)
		МассивСвязей = Новый Массив;
		Для Каждого Связь Из ИсточникДанных.СвязиПараметровВыбора Цикл
			МассивСвязей.Добавить(Новый СвязьПараметраВыбора(Связь.Имя, "Элементы." + ИмяКомпонента + ".ТекущиеДанные." + Связь.ПутьКДанным));
		КонецЦикла;
		Возврат Новый ФиксированныйМассив(МассивСвязей);
	КонецФункции
	
	Функция ОписаниеКолонкиИсходныхЗначений(ИсточникДанных, ИмяКомпонента)
		ОписаниеКолонки = ОписаниеКолонки(ИсточникДанных, ИмяКомпонента);
	...
	Функция АктуальныеКолонкиРегистра(НастройкиКомпонента) 
		...
		//Для регистров накопления и регистров сведений, подчиненных регистратору, измерения не должны являться измерениями (набором значений, уникально идентифицирующим строку).
		//За это отвечает НомерСтроки.		
		ИзмеренияРегистраЯвляютсяИзмерениямиТаблицы = НастройкиКомпонента.ТипМетаданных = "регистрсведений" И НастройкиКомпонента.НезависимыйРегистрСведений; 
		Для Каждого Колонка Из МетаданныеОбъекта.Измерения Цикл
			...
			АктуальныеКолонки.Добавить(НовыйСтруктураКолонки(Колонка, ИзмеренияРегистраЯвляютсяИзмерениямиТаблицы));
		КонецЦикла;
	...
	Функция ПодставитьУсловияВТекстЗапроса(Запрос, Форма, ИмяКомпонента, НастройкиКомпонента)
		...
		Для Каждого Пара Из НастройкиКомпонента.Ключи Цикл
			Условия.Добавить("Таблица." + Пара.Ключ + " = &Ключ__" + Пара.Ключ);
			Запрос.УстановитьПараметр("Ключ__" + Пара.Ключ, Пара.Значение);
		КонецЦикла;
		
		Для Каждого Отбор Из НастройкиКомпонента.Отборы Цикл
			Если НЕ ЗначениеЗаполнено(Форма[ИмяПоляОтбора(ИмяКомпонента, Отбор)]) Тогда Продолжить; КонецЕсли;
			Условия.Добавить("Таблица." + Отбор + " = &Отбор__" + Отбор);
			Запрос.УстановитьПараметр("Отбор__" + Отбор, Форма[ИмяПоляОтбора(ИмяКомпонента, Отбор)]);
		КонецЦикла;
...
Функция КоллекцияДанныхДляСохранения(Форма, ИмяКомпонента) 
	...
	Для Сч = 0 По Таблица.Количество() - 1 Цикл
		...
		Если ИзмеренияИзменены И Настройки.ТипМетаданных = "регистрсведений" Тогда 
...
#Область Автонумерация 
#Если Клиент Тогда
	
	Функция АвтонумерацияПриНачалеРедактирования(Форма, Элемент, НоваяСтрока, Копирование) Экспорт
		Если НоваяСтрока Тогда
			ИмяКомпонента = Элемент.Имя; 
			Настройки = А1Э_УниверсальнаяФорма.НастройкиКомпонента(Форма, ИмяКомпонента);
			ИмяКолонкиНомерСтроки = Настройки.Контекст.Автонумерация__ИмяРеквизита;
			МаксимальныйНомерСтроки = 0;
			Для Каждого Строка Из Форма[ИмяКомпонента] Цикл
				НомерТекущейСтроки =  Строка[ИмяКолонкиНомерСтроки];
				Если НомерТекущейСтроки > МаксимальныйНомерСтроки Тогда 
					МаксимальныйНомерСтроки = НомерТекущейСтроки; 
				КонецЕсли;	
			КонецЦикла;
			Элемент.ТекущиеДанные[ИмяКолонкиНомерСтроки] = МаксимальныйНомерСтроки + 1;	
		КонецЕсли;
	КонецФункции	
