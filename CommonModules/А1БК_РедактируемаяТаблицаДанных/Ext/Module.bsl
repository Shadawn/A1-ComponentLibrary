#Если НЕ Клиент Тогда
	...
	Функция ДобавитьОписание(МассивОписаний, ИмяКомпонента, Знач ИмяМетаданных, РодительЭлемента = Неопределено, ПередЭлементом = Неопределено, Параметры = Неопределено, Действия = Неопределено) Экспорт
		ЧастиИмени = А1Э_Строки.ПередПосле(ИмяМетаданных, ".");
		Если НРЕГ(ЧастиИмени.Перед) <> "регистрсведений" Тогда
			А1Э_Служебный.СлужебноеИсключение("РедактируемаяТаблицаДанных: в текущей версии поддерживаются только регистры сведений!");
		КонецЕсли;
		...
		"Отборы", А1Э_Массивы.Массив(А1Э_Структуры.ЗначениеСвойства(Параметры, "Отборы", Новый Массив)),
		...
		Для Каждого КолонкаРегистра Из АктуальныеКолонкиРегистра(НастройкиКомпонента) Цикл
			Колонки.Добавить(ОписаниеКолонки(КолонкаРегистра.Метаданные));
			Колонки.Добавить(ОписаниеКолонкиИсходныхЗначений(КолонкаРегистра.Метаданные));
			НастройкиКомпонента.Колонки.Добавить(А1Э_Структуры.Создать(
			"Имя", КолонкаРегистра.Метаданные.Имя,
			"Тип", КолонкаРегистра.Метаданные.Тип,
			"Заголовок", КолонкаРегистра.Метаданные.Синоним,
			"ИмяИсходногоЗначения", ИмяКолонкиИсходногоЗначения(КолонкаРегистра.Метаданные.Имя),
			"ЭтоИзмерение", КолонкаРегистра.ЭтоИзмерение,
			));
		КонецЦикла;
...
#Если Клиент Тогда
	...
	Функция ФормаПередЗакрытием(ИмяКомпонента, Форма, Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка) Экспорт
		...
		Попытка
			А1Э_ОбщееСервер.РезультатФункции("А1БК_РедактируемаяТаблицаДанных.СохранитьКоллекцию", КоллекцияДанныхДляСохранения, Настройки);
		Исключение
...
#Если НЕ Клиент Тогда
	...
	Функция АктуальныеКолонкиРегистра(НастройкиКомпонента) 
		...
		//Колонки = А1Э_Метаданные.КолонкиРегистра(МетаданныеОбъекта);
