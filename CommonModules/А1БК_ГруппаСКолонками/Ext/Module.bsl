...
//   - 
//
Функция ДобавитьОписание(МассивОписаний, ИмяКомпонента, КоличествоКолонок = 2, Заголовок = "", РодительЭлемента = Неопределено, ПередЭлементом = Неопределено, Параметры = Неопределено, Действия = Неопределено) Экспорт
...
#Если НЕ Клиент Тогда
	...
	Функция РаспределитьПодчиненные(Форма, ИмяКомпонента) Экспорт
		...
		Для Каждого Элемент Из ГоловнойЭлемент.ПодчиненныеЭлементы Цикл
			Если СтрНачинаетсяС(Элемент.Имя, ИмяКомпонента) И ТипЗнч(Элемент) = Тип("ГруппаФормы") Тогда 
